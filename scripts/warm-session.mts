...
import { getSession, setWarmState } from "@/db/queries/sessions";
...
try {
  ...
  warmSession(db, id, basketOnly);

  setWarmState(db, id, "ready");
  console.log(
  ...
  try {
    setWarmState(drizzle(sqlite), id, "failed", message);
  } catch {
